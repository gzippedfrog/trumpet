        ...
        <div class="flex mt-3 gap-3">
            <?php if (isset($_SESSION['id'])): ?>
                ...
                <?php if (!($isReply ?? false)): ?>
                    <a href="/posts/create?parent_id=<?= $post['id'] ?>&page=<?= $_GET['page'] ?>"
                        class="flex items-center hover:text-primary-500 dark:hover:text-primary-400">
                        Reply
                        <svg fill="none" stroke="currentColor" stroke-width="1.5" width="18" height="18" viewBox="0 0 24 24"
                ...
                <?php if ($_SESSION['id'] === $post['author_id']): ?>
                    <a href="/posts/edit?id=<?= $post['id'] ?>&page=<?= $_GET['page'] ?>"
                    ...
                    <form method="POST" action="/posts" class="hover:text-red-600 dark:hover:text-red-400">
                ...
                <form method="POST" action="/like" class="ml-auto ">
                    ...
                    <button type="submit" class="flex items-center hover:text-pink-400 dark:hover:text-pink-300">
                        ...
                        <svg aria-hidden="true" fill="<?= $post["liked"] ? "currentColor" : "none" ?>" stroke="currentColor"
                            stroke-width="1.5" viewBox="0 0 24 24" width="16" height="16" xmlns="http://www.w3.org/2000/svg"
                            class="ml-1">
                            <path
