<div
    class="mb-3 p-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 <?= isset($isReply) ? 'ml-10' : '' ?>">
    ...
    <div class="flex mt-3 gap-3">
        <?php if (isset($_SESSION['id'])): ?>
            ...
            <?php if (!isset($isReply)): ?>
                <a href="/posts/create?parent_id=<?= $post['id'] ?>"
                    class="flex items-center hover:text-blue-700 dark:text-white">
                    Reply
            ...
            <?php if ($_SESSION['id'] === $post['author_id']): ?>
                ...
                <form method="POST" action="/posts" class="hover:text-red-700 dark:text-white">
                    ...
                    <button type="submit" class="flex items-center">Delete<svg fill="none" stroke="currentColor"
                            stroke-width="2" width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                            aria-hidden="true" class="inline ml-1">
                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <form method="POST" action="/like" class="ml-auto dark:text-white">
                ...
                <input type="hidden" name="id" value=<?= $post["id"] ?>>
                <button type="submit" class="flex items-center hover:text-blue-700">
                    Like
