...
<?php if (isset($_SESSION['username'])) : ?>
    <form method="POST" class="flex flex-col gap-5 mb-6">
        ...
        <textarea name="body" rows="4" placeholder="Write your thoughts here..." class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
...
<?php foreach ($posts as $post) : ?>
    <div class="mb-3 p-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
        <div class="font-bold text-black dark:text-white"><?= $post["author_username"] ?></div>
        <div class="mb-3 font-normal text-gray-700 dark:text-white"><?= $post["body"] ?></div>
        <a href="like/<?= $post['id'] ?>" class="flex gap-1 items-center">
